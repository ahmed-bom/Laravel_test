...
background: -moz-linear-gradient(-45deg, rgba(30,29,31,1) 0%, rgb(109, 95, 97) 100%);
background: -webkit-gradient(left top, right bottom, color-stop(0%, rgba(30,29,31,1)), color-stop(100%, rgb(74, 70, 71)));
background: -webkit-linear-gradient(-45deg, rgba(30,29,31,1) 0%, rgb(165, 160, 161) 100%);
background: -o-linear-gradient(-45deg, rgba(30,29,31,1) 0%, rgb(148, 135, 137) 100%);
background: -ms-linear-gradient(-45deg, rgba(30,29,31,1) 0%, rgb(112, 107, 108) 100%);
background: linear-gradient(135deg, rgba(30,29,31,1) 0%, rgb(126, 115, 117) 100%);
...
.box-info {
  ...
  position: absolute;
	...
	background-color: rgba(253, 253, 253, 0.833);
...
.b-support:hover, .b-cta:hover {
  color: #fff;
	background-color: #574f52;
	border: #6c696a 1px solid;
}
.fieldset-body {
    display: table;
}
...
<body>
  ...
  function openLoginInfo(reverse = false) {
  $('.b-form').css("opacity", "0.01");

  if (reverse) {
    // Reverse animation for login
    $('.box-form').animate({ left: "0px" }, 300);  // Back to original position
    $('.box-info').animate({ right: "-5px" }, 300); // Back to original position
  } else {
    // Normal animation for showing login info
    $('.box-form').animate({ left: "-37%" }, 300);
    $('.box-info').animate({ right: "-37%" }, 300);
  }
}

function closeLoginInfo(reverse = false) {
  $('.b-form').css("opacity", "1");

  if (reverse) {
    // Reverse animation for login
    $('.box-form').animate({ left: "-37%" }, 300);  // Move offscreen left
    $('.box-info').animate({ right: "-37%" }, 300); // Move offscreen right
  } else {
    // Normal animation for hiding login info
    $('.box-form').animate({ left: "0px" }, 300);   // Reset to original position
    $('.box-info').animate({ right: "-5px" }, 300); // Reset to original position
  }
}
...
function showSignupForm() {
  ...
  closeLoginInfo(true); // Reverse animation to hide login info
}

function showLoginForm() {
  $('#signup_box').hide(); // Hide signup box
  $('#login_box').show(); // Show login box
  openLoginInfo(true); // Reverse animation to show login info again
  setTimeout(() => {
      $('.b-form').css("opacity", "1"); // Keep the "More Info" button visible
  }, 300); // Timing should match the animation duration
}
